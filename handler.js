            ...
            if (chat.welcome) {
              ...
              for (let user of participants) {
                ...
                try {
                  ...
                  pp = 'https://i.imgur.com/KRd67s7.jpg'; // Assign default image URL
                  ppgp = 'https://i.imgur.com/qcOtNSv.jpg'; // Assign default image URL
                } finally {
                  ...
                  try {
                    ...
                    this.sendMessage(id, {
                        ...
                        externalAdReply: {
                        title: "𝐓𝐡𝐞 𝐔𝐦𝐚𝐫 𝐁𝐨𝐭",
                        body: "welcome to Channel",
                        thumbnailUrl: welcomeApiUrl,
                        sourceUrl: 'https://whatsapp.com/channel/0029VaEphvR3LdQbvGs4F50Z',
                        mediaType: 1,
            ...
            if (chat.welcome) {
              ...
              for (let user of participants) {
                ...
                try {
                  ...
                  let text = (chat.sBye || this.bye || conn.bye || 'HELLO JAWN GET OUT AND DON'T COME BACK 😹♥️, @user')
                    .replace('@user', '@' + user.split('@')[0]);
...
export async function groupsUpdate(groupsUpdate) {
    ...
    for (const groupUpdate of groupsUpdate) {
        ...
        if (groupUpdate.desc) {
            text = (chats.sDesc || this.sDesc || conn.sDesc || `*${emoji.desc} Description has been changed By UD-MD BOT to*\n@desc`)
                .replace("@desc", groupUpdate.desc)
        } else if (groupUpdate.subject) {
            text = (chats.sSubject || this.sSubject || conn.sSubject || `*${emoji.subject} Subject has been changed By UD-MD BOT to*\n@subject`)
                .replace("@subject", groupUpdate.subject)
        } else if (groupUpdate.icon) {
            text = (chats.sIcon || this.sIcon || conn.sIcon || `*${emoji.icon} Icon has been changed by UD-MD BOT 💗👀*`)
                .replace("@icon", groupUpdate.icon)
        } else if (groupUpdate.revoke) {
            text = (chats.sRevoke || this.sRevoke || conn.sRevoke || `*${emoji.revoke} Group link has been changed By UD-MD BOT to*\n@revoke`)
                .replace("@revoke", groupUpdate.revoke)
        } else if (groupUpdate.announce === true) {
            text = (chats.sAnnounceOn || this.sAnnounceOn || conn.sAnnounceOn || `*${emoji.announceOn} Group is now closed By UD-MD BOT 💗👀!*`)
        } else if (groupUpdate.announce === false) {
            text = (chats.sAnnounceOff || this.sAnnounceOff || conn.sAnnounceOff || `*${emoji.announceOff} Group is now open By UD-MD BOT 💗👀!*`)
...
export async function presenceUpdate(presenceUpdate) {
    ...
    if (user?.afk && status === "composing" && user.afk > -1) {
        if (user.banned) {
            user.afk = -1;
            user.afkReason = "Mother fucker Banned Afk";
            return;
        }
...
global.dfail = (type, m, conn) => {
    ...
    ${userTag} Hey Janu This command can only be used by the *Bot Owner*!`,
        moderator: `*${emoji.moderator} Moderator's Query*\n
    ${userTag} Hey Janeman This command can only be used by *Moderators*!`,
        premium: `*${emoji.premium} Premium Query*\n
    ${userTag} Hello Bhai This command is only for *Premium Members*!`,
        group: `*${emoji.group} Group Query*\n
    ${userTag} Hello Gandu This command can only be used in *Group Chats*!`,
        private: `*${emoji.private} Private Query*\n
    ${userTag} Hi Sis This command can only be used in *Private Chats*!`,
        admin: `*${emoji.admin} Admin's Query*\n
    ${userTag} Hey Bro This command is only for *Group Admins*!`,
        botAdmin: `*${emoji.botAdmin} Bot Admin's Query*\n
    ${userTag} Hey I LovE YoU Now Make the bot an *Admin* to use this command!`,
        unreg: `*${emoji.unreg} Registration Query*\n
    ${userTag} Hey Lowlli Please register to use this feature by typing:\n\n*#register name.age*\n\nExample: *#register ${m.name}.18*!`,
    ...
    ${userTag} Mf This feature is *disabled*!`,
    }
     [type]
