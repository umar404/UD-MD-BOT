...
let handler = async (m, { conn, usedPrefix, command }) => {
	...
	if (m.sender in conn.cartoon)
		throw "Jawn There is still an unfinished process, my son. Please wait until it's over. >//<";
	let q = m.quoted ? m.quoted : m;
	let mime = (q.msg || q).mimetype || q.mediaType || "";
	if (!mime) throw `Hey Jawn Where is the picture you want to convert to a cartoon?🤔`;
	if (!/image\/(jpe?g|png)/.test(mime)) throw `Sorry Jawn file ${mime} not supported`;
	else conn.cartoon[m.sender] = true;
	m.reply("Jawn Please Wait a Moment Converting The Picture To Cartoon 🙂");
		...
		Cartoon(img).then(async (response) => {
			if (response.message == "success") {
				await conn.sendFile(
					...
					"",
					"Jawn Here Is Your To Cartoon Converted Picture ♥ >//<",
					m
				);
				...
				m.reply(
					"Excuse me my jawn, the picture does not reveal a face, please send a picture in which the face is exposed and visible."
				);
			}
		});
	} catch {
